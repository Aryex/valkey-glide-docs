...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    // Create a cache configuration
    var cache = new ClientSideCacheConfig(
        maxCacheKb: 1024,                             // 1 MB maximum cache size
        entryTtl: TimeSpan.FromSeconds(60)            // 60 second TTL per entry
    )
    ...
    // Cluster client
    var clusterConfig = new ConnectionConfiguration.ClusterClientConfigurationBuilder()
        .WithAddress("localhost", 6379)
        .WithClientSideCache(cache)
        .Build();
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ```csharp
    // Get metrics
    double hitRate = await client.GetCacheHitRateAsync();         // double 0.0–1.0
    double missRate = await client.GetCacheMissRateAsync();       // double 0.0–1.0
    long entryCount = await client.GetCacheEntryCountAsync();     // long
    long evictions = await client.GetCacheEvictionsAsync();       // long
    long expirations = await client.GetCacheExpirationsAsync();   // long
    long total = await client.GetCacheTotalLookupsAsync();        // long
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    // Both clients share the same cache instance
    // Each ClientSideCacheConfig has a unique auto-generated cache ID,
    // so clients must use the *same object* (not just same config values)
    var cache = new ClientSideCacheConfig(
        maxCacheKb: 1024,
        entryTtl: TimeSpan.FromSeconds(60)
    );

    var config1 = new ConnectionConfiguration.StandaloneClientConfigurationBuilder()
        .WithAddress("localhost", 6379)
        .WithClientSideCache(cache)
        .Build();
    var client1 = await GlideClient.CreateClientAsync(config1);

    var config2 = new ConnectionConfiguration.StandaloneClientConfigurationBuilder()
        .WithAddress("localhost", 6379)
        .WithClientSideCache(cache)
        .Build();
